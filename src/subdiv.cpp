...
int Subdiv::nextEdge(int edge) const
{
    CV_DbgAssert((size_t)(edge >> 2) < qedges.size());
    return qedges[edge >> 2].next[edge & 3];
}

int Subdiv::rotateEdge(int edge, int rotate) const
{
    return (edge & ~3) + ((edge + rotate) & 3);
}

int Subdiv::symEdge(int edge) const
{
    return edge ^ 2;
}

int Subdiv::getEdge(int edge, int nextEdgeType) const
{
    CV_DbgAssert((size_t)(edge >> 2) < qedges.size());
...
int Subdiv::edgeOrg(int edge, Point2f* orgpt) const
{
    CV_DbgAssert((size_t)(edge >> 2) < qedges.size());
...
int Subdiv::edgeDst(int edge, Point2f* dstpt) const
{
    CV_DbgAssert((size_t)(edge >> 2) < qedges.size());
...
Point2f Subdiv::getVertex(int vertex, int* firstEdge) const
{
...
Subdiv::QuadEdge::QuadEdge()
{
    next[0] = next[1] = next[2] = next[3] = 0;
    pt[0] = pt[1] = pt[2] = pt[3] = 0;
}

Subdiv::QuadEdge::QuadEdge(int edgeidx)
{
...
bool Subdiv::QuadEdge::isfree() const
{
    return next[0] <= 0;
}

Subdiv::Vertex::Vertex()
{
    firstEdge = 0;
    type = -1;
}

Subdiv::Vertex::Vertex(Point2f _pt, bool _isvirtual, int _firstEdge)
{
...
bool Subdiv::Vertex::isvirtual() const
{
    return type > 0;
}

bool Subdiv::Vertex::isfree() const
{
    return type < 0;
}

void Subdiv::splice( int edgeA, int edgeB )
{
...
void Subdiv::setEdgePoints(int edge, int orgPt, int dstPt)
{
...
int Subdiv::connectEdges( int edgeA, int edgeB )
{
    int edge = newEdge();
...
void Subdiv::swapEdges( int edge )
{
...
int Subdiv::isRightOf(Point2f pt, int edge) const
{
    Point2f org, dst;
    edgeOrg(edge, &org);
    edgeDst(edge, &dst);
...
void Subdiv::deleteEdge(int edge)
{
...
int Subdiv::newPoint(Point2f pt, bool isvirtual, int firstEdge)
{
    if( freePoint == 0 )
    {
...
void Subdiv::deletePoint(int vidx)
{
    CV_DbgAssert( (size_t)vidx < vtx.size() );
...
int Subdiv::locate(Point2f pt, int& _edge, int& _vertex)
{
...
int Subdiv::insert(Point2f pt)
{
    ...
    if( location == PTLOC_ON_EDGE )
    {
        ...
        ;
    // else
    //     CV_Error_(CV_StsError, ("Subdiv::locate returned invalid location = %d", location) );
...
void Subdiv::insert(const std::vector<Point2f>& ptvec)
{
    for( size_t i = 0; i < ptvec.size(); i++ )
        insert(ptvec[i]);
}

void Subdiv::initDelaunay( Rect rect )
{
...
int Subdiv::findNearest(Point2f pt, Point2f* nearestPt)
{
    if( !validGeometry )
        calcVoronoi();
...
void Subdiv::getEdgeList(std::vector<Vec4f>& edgeList) const
{
    edgeList.clear();
...
int Subdiv::getEdgeVtx1(int edgeidx, Point2f* pt)
{
    if(pt)
    {
        *pt = vtx[qedges[edgeidx].pt[0]].pt;
    }
    return qedges[edgeidx].pt[0] - 4;
}

int Subdiv::getEdgeVtx2(int edgeidx, Point2f* pt)
{
    if(pt)
    {
        *pt = vtx[qedges[edgeidx].pt[2]].pt;
    }
    return qedges[edgeidx].pt[2] - 4;
}

void Subdiv::getEdgeIdxList(std::vector<int>& edgeIdxList) const
{
    edgeIdxList.clear();
...
void Subdiv::getLeadingEdgeList(std::vector<int>& leadingEdgeList) const
{
...
void Subdiv::getTriangleList(std::vector<Vec6f>& triangleList) const
{
    triangleList.clear();
    int i, total = (int)(qedges.size()*4);
    std::vector<bool> edgemask(total, false);
...
void Subdiv::getVoronoiFacetList(const std::vector<int>& idx,
